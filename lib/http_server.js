...
module.exports = {
	...
	initialize: function() {
		...
		http.createServer(function(req, res) {
			...
			else if (uri[1] == 'random') {
				res.writeHead(200, {'Content-Type': 'application/json'});
				let response = JSON.stringify(markov.random());
				console.log(colors.cyan('random word object requested'));
				console.log(response);
				res.end(response);
			}

			else if (uri[1] == 'random_seed') {
				res.writeHead(200, {'Content-Type': 'text/plain'});
				let response = markov.random_seed();
				console.log(colors.cyan('random seed requested : ' + response));
				res.end(response);
			}
